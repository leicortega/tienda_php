...
$sql = $conexion->prepare('SELECT * from categorias  order by categorias.id '.$_REQUEST['order'].' limit '.$inicio.', 5');
$sql->execute();
$datos = $sql->fetchAll();
// Consulta para obtener el total de los registros para hayar el numero de paginas
$sql_item = $conexion->prepare('SELECT * from categorias');
$sql_item->execute();
            ...
            <?php foreach ($datos as $dato) { ?>
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-warning waves-effect waves-light" onclick="editar_categorias(<?php echo $dato[0]; ?>)"><i class="mdi mdi-pencil"></i></button> 
                        <button type="button" class="btn btn-danger waves-effect waves-light" onclick="eliminar_categorias(<?php echo $dato[0]; ?>)"><i class="mdi mdi-delete"></i></button>
                    </td>
                </tr>
            <?php } ?>
    ...
    <nav aria-label="..." class="text-center mt-3">
        <ul class="pagination">
            <li class="page-item <?php echo $_REQUEST['page'] > 1 ? '' : 'disabled'; ?>">
                ...
                <a class="page-link" href="#" tabindex="-1" onclick="mostrar_tabla_categorias(<?php echo ($_REQUEST['page'] - 1); ?>, 'desc')">Atras</a>
            </li>
            ...
            for ($i = $j; $i <= $num_item_paginador; $i++) {  ?>
                ...
                <li class="page-item <?php  echo $i == $_REQUEST['page'] ? 'active' : ''; ?>"><a class="page-link" href="#" onclick="mostrar_tabla_categorias(<?php echo $i; ?>, 'desc')"><?php echo$i; ?></a></li>
            <?php } ?>
            ...
            <li class="page-item <?php echo $_REQUEST['page'] == $num_item ? 'disabled' : ''; ?>">
                <a class="page-link" href="#" onclick="mostrar_tabla_categorias(<?php echo ($_REQUEST['page'] + 1); ?>, 'desc')">Siguiente</a>
            </li>
        </ul>
    </nav>
<?php 
